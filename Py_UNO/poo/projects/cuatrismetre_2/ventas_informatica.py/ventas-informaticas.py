...
class GestionVentas:
    ...
    def producto_mas_vendido(self) -> list:
        ...
        for cp, cv in aux.items():
            if cv == max_cv:
                max_vendidos.append((cp, cv))
        return max_vendidos


def main():
    gvi = GestionVentas()
    gvi.leer_ventas(r"C:\Users\Alumnos\Desktop\Python_Objetos\Py_UNO\poo\projects\cuatrismetre_2\ventas_informatica.py\ventas.txt")
    gvi.mostrar_ventas()
    print("---")
    ...
    print("--- Ventas por Vendedor Detallado ---")
